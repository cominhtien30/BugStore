...
<?php 
	...
	class product
	{
		...
		public function Show_Product($searchpost,$searchget){
			...
			$star_page=($page-1)*$product_in_page;
			$query = "SELECT A.productName,A.image,A.size,A.status, C.brandName,B.catName,A.price,A.type,A.description FROM tbl_product A, tbl_category B, tbl_brand C WHERE ".$searchpost." ".$searchget."  A.catId=B.catId AND A.brandId=C.brandId GROUP BY A.productName, A.description, A.price  ORDER BY A.productName  DESC LIMIT ".$star_page.",".$product_in_page."";
			$result = $this->db->select($query);
			return $result;
		}
		public function Show_All_Page(){
			$query="SELECT * FROM tbl_product group by productName";
			$result=$this->db->select($query);
			return $result;
		}
		public function Show_ProductAdmin(){
			$query = "SELECT A.productName,A.status,A.image,A.size, C.brandName,B.catName,A.price,A.type,A.description FROM tbl_product A, tbl_category B, tbl_brand C WHERE   A.catId=B.catId AND A.brandId=C.brandId GROUP BY A.productName, A.description, A.price  ORDER BY A.productName  DESC";
			$result = $this->db->select($query);
			return $result;
		}
		...
		public function Get_ProductFeathered(){
			$query = "SELECT  A.productName,A.status, C.brandName,B.catName,A.price,A.image,A.type,A.description
		...
		public function get_ProductNew(){
			$query = "SELECT A.productId,A.status, A.productName, C.brandName,B.catName,A.price,A.image,A.type,A.description
					  FROM tbl_product A, tbl_category B, tbl_brand C
		...
		public function update_status($status,$name_pro){
			
				$query = "UPDATE tbl_product SET status = '$status' WHERE productName = '$name_pro' ";
				$result = $this->db->update($query);
				if($result){
					$alert = "<span class='text-success' >Update thành công</span";
					return $alert;
